...
func (r *readerImpl) read(dst []byte, off int64) (int64, int, error) {
	...
	r.logger.Debug("decompressed",
		Uint64("offsetWithinFrame", offsetWithinFrame),
		Uint64("end", offsetWithinFrame+size),
		Uint64("size", size),
		Int("lenDecompressed", len(decompressed)),
		Int("lenDst", len(dst)),
		Any("index", index))
...
func (r *readerImpl) indexFooter() (*btree.BTreeG[*env.FrameOffsetEntry], *env.FrameOffsetEntry, error) {
	...
	r.logger.Debug("loaded", Any("footer", &footer))
